...
void Estimator::updateGNSSStatistics()
{
    ...
    ecef_pos = anc_ecef + R_ecef_enu * enu_pos;
        ...
        // 打开文件并写入矩阵数据
    std::ofstream outfile("/home/nvidia/gvins_ws/R_ecef_enu_local.txt");

    if (outfile.is_open())
    {
        // 写入 R_enu_local 矩阵到文件第一行
        for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 3; ++j)
            {
                outfile << R_enu_local(i, j);
                if (i != 2 || j != 2)
                    outfile << " ";
            }
        }
        ...
        // 写入 R_ecef_enu 矩阵到文件第二行
        for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 3; ++j)
            {
                outfile << R_ecef_enu(i, j);
                if (i != 2 || j != 2)
                    outfile << " ";
            }
        }
    ...
    {
        std::cerr << "Unable to open file R_ecef_enu_local.txt for writing" << std::endl;
    }
}
