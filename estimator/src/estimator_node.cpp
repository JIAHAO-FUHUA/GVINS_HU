...
#define MAX_GNSS_CAMERA_DELAY 1.0
...
{
    sensor_msgs::ImuPtr tmp_imu(new sensor_msgs::Imu(*imu_msg));
    // tmp_imu->linear_acceleration.x = imu_msg->linear_acceleration.z;
    // tmp_imu->linear_acceleration.y = imu_msg->linear_acceleration.x;
    // tmp_imu->linear_acceleration.z = imu_msg->linear_acceleration.y;
    // tmp_imu->angular_velocity.x = imu_msg->angular_velocity.z;
    // tmp_imu->angular_velocity.y = imu_msg->angular_velocity.x;
    // tmp_imu->angular_velocity.z = imu_msg->angular_velocity.y;

    std::string write_path1 = "/home/nvidia/gvins_ws/src/GVINS/data/acc.txt";
    std::ofstream outfile1;
    outfile1.open(write_path1, std::ofstream::app);
    outfile1 << setprecision(19) << tmp_imu->header.stamp.toSec() << " "
             << tmp_imu->linear_acceleration.x << " "
             << tmp_imu->linear_acceleration.y << " "
             << tmp_imu->linear_acceleration.z << " " << 0 << " "
             <<0<< " " << 0 << " " << 1 << std::endl;
    outfile1.close();
    std::string write_path2 = "/home/nvidia/gvins_ws/src/GVINS/data/gyro.txt";
    std::ofstream outfile2;
    outfile2.open(write_path2, std::ofstream::app);
    outfile2 << setprecision(19) << tmp_imu->header.stamp.toSec() << " "
             << tmp_imu->angular_velocity.x << " "
             << tmp_imu->angular_velocity.y << " "
             << tmp_imu->angular_velocity.z << " " << 0 << " "
             << 0 << " " << 0 << " " << 1 << std::endl;
    outfile2.close();

    if (tmp_imu->header.stamp.toSec() <= last_imu_t)
    {
        ROS_WARN("imu message in disorder!");
        return;
    }
    last_imu_t = tmp_imu->header.stamp.toSec();

    m_buf.lock();
    imu_buf.push(tmp_imu);
    m_buf.unlock();
    con.notify_one();
...

    {
        ...
        predict(tmp_imu);
        std_msgs::Header header = tmp_imu->header;
