...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
      ...
      quaternion.w() = odometry.pose.pose.orientation.w;
        ...
        // 创建表示绕X轴逆时针旋转90度的四元数
    double angle_x = -90.0 * M_PI / 180.0; // 逆时针旋转90度，转换为弧度
    Eigen::Quaterniond rotationQuaternion_x(Eigen::AngleAxisd(angle_x, Eigen::Vector3d::UnitX()));

    // 将旋转应用到原始姿态四元数
    Eigen::Quaterniond rotatedQuaternion = quaternion * rotationQuaternion_x;
    quaternion = rotatedQuaternion;
     ...
     Eigen::Matrix3d rotation_odo = quaternion.toRotationMatrix();

      Eigen::Vector3d eulerAngles2 = rotation_odo.eulerAngles(0,1,2);

      Eigen::Vector3d eulerAnglesDegrees2 = eulerAngles2 * (180.0/3.14159);
      std::cout <<"pixpose:"<< eulerAnglesDegrees2 <<std::endl;
      ...
      rotation_matrix3 << 0.0102796, 0.999926, 0.00653913, -0.046647, -0.00605282,
          0.998893, 0.998859, -0.0105733, 0.0465816;
      Eigen::Vector3d tm3;
      tm3 << -0.0477312, 0.0495292, -0.065652;
    ...
    rotationZ.block<3,3>(0,0) = Eigen::AngleAxisd(angle_z, Eigen::Vector3d::UnitZ()).toRotationMatrix();
      ...
      // transform.transform.rotation = tf2::toMsg(rotation_quaternion);
